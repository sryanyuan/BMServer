...
GameScene::GameScene()
{
	...
	m_dwInstanceMapFreeTime = GetTickCount();
...
bool GameScene::Initialize(DWORD _dwMapResID, DWORD _dwMapID)
{
	...
	bool bRet = true;
	m_dwMapID = _dwMapID;
	m_dwMapResID = _dwMapResID;
	MapConfigManager& xMapConfigManager = GameSceneManager::GetInstance()->GetMapConfigManager();
	const LuaMapInfo* pMapInfo = xMapConfigManager.GetLuaMapInfo(_dwMapResID);
	if (NULL == pMapInfo)
	{
		return false;
	}
	...
	if(!LoadScript())
	{
		LOG(ERROR) << "运行[" << "map script " << _dwMapResID << "]失败";
		return false;
	}
	else
	{
#ifdef _DEBUG
		LOG(INFO) << "载入地图脚本文件[" << _dwMapResID << "]成功";
#endif
		int nRet = 0;
		lua_getglobal(m_xScript.GetVM(), "OnMapLoaded");
		if (LUA_TFUNCTION != lua_type(m_xScript.GetVM(), -1))
		{
			LOG(ERROR) << "Can't find lua callback OnMapLoaded";
		...
		{
#ifdef _DEBUG
			LOG(INFO) << "执行" << pMapInfo->szMapResFile << "[OnMapLoaded]脚本函数";
#endif
		}
		else
		{
...
void GameScene::Release()
{
	...
	if(m_pCellData)
	{
		for(int i = 0; i < m_stMapInfo.nRow; ++i)
		{
			for(int j = 0; j < m_stMapInfo.nCol; ++j)
			{
				CELLDATALIST* pCellList = m_pCellData[j + i * m_stMapInfo.nCol].pCellObjects;
				if (NULL != pCellList)
				{
					CELLDATALIST::const_iterator it = pCellList->begin();
					for (it ; it != pCellList->end(); ++it)
					{
						CellData* pData = *it;
						if (NULL != pData)
						{
...
bool GameScene::GetSceneData(HeroObject* _pObj)
{
		...
		++iter)
	{
		...
		if(iter->second->GetID() != _pObj->GetID())
		{
			...
			// check weapon item level
			not.dwSpeLook1 = 0;
			if (0 != GETITEMATB(pHero->GetEquip(PLAYER_ITEM_WEAPON), ID))
			{
				int nWeaponSpecLevel = GetItemUpgrade(GETITEMATB(pHero->GetEquip(PLAYER_ITEM_WEAPON), Level));
				if (nWeaponSpecLevel >= 5)
				{
					not.dwSpeLook1 |= 0x1;
				}
			}
...
GameObject* GameScene::GetPlayerWithoutLockInt(int _dwID)
{
	RECORD_FUNCNAME_WORLD;
	...
	if(m_xPlayers.find(_pObj->GetID()) == m_xPlayers.end())
	{
		...
		else
		{
			if(_pObj->GetType() == SOT_HERO)
			{
				...
				// check weapon item level
				if (0 != pHero->GetEquip(PLAYER_ITEM_WEAPON)->id)
				{
					int nWeaponSpecLevel = GetItemUpgrade(pHero->GetEquip(PLAYER_ITEM_WEAPON)->level);
					if (nWeaponSpecLevel >= 5)
					{
						not.dwSpeLook1 |= 0x1;
					}
...
bool GameScene::RemovePlayer(DWORD _dwID, bool _bDelete /* = true */)
{
	...
	if(_bDelete)
	{
		...
		if (CMainServer::GetInstance()->GetServerMode() == GM_LOGIN)
		{
			...
			{
				protocol::MPlayerDisconnectedNtf ntf;
				ntf.set_lid(pHero->GetLSIndex());
				ntf.set_uid(pHero->GetUID());
				SendProtoToServer(CMainServer::GetInstance()->GetLSConnIndex(), protocol::PlayerDisconnectedNtf, ntf);
			}
		}
...
bool GameScene::InsertNPC(GameObject* _pNPC)
{
	...
	if(iter == m_xNPCs.end())
	{
			...
			_pNPC->GetType() == SOT_NPC)
		{
			...
			AddMappedObject(_pNPC->GetUserData()->wCoordX,
				_pNPC->GetUserData()->wCoordY, CELL_MOVEOBJECT, _pNPC);
...
bool GameScene::RemoveNPC(DWORD _dwID, bool _bDelete /* = true */)
{
	...
	if(iter != m_xNPCs.end())
	{
		...
		if(!RemoveMappedObject(pObj->GetValidPositionX(), pObj->GetValidPositionY(), pObj))
		{
			// 假如是FlyToMap的，映射数据已经删过一次了，这里没删掉正常
			if(_bDelete)
			{
				char szName[20];
				ObjectValid::GetItemName(&pObj->GetUserData()->stAttrib, szName);
...
void GameScene::Update(DWORD _dwTick)
{
	...
	m_nCurMonsters = 0;
	{
		for(std::map<DWORD, GameObject*>::const_iterator iter = m_xNPCs.begin();
			...
			++iter)
		{
			GameObject* pUpdateObject = iter->second;
			if(pUpdateObject->GetType() == SOT_MONSTER &&
				pUpdateObject->GetUserData()->eGameState != OS_DEAD)
			{
				MonsterObject* pMonster = static_cast<MonsterObject*>(pUpdateObject);
				if(pMonster->GetMaster() == NULL)
				{
					// it is normal monster, record count
					++m_nCurMonsters;

					if (pMonster->GetUserData()->eGameState != OS_DEAD)
					{
						// it is alive
						m_xMonsCountRecorder.Inc(pUpdateObject->GetObject_ID(), 1);
					}
				}
			}
			...
			iter->second->DoWork(_dwTick);
			// alarm object update cost too long
			if (xMonsTimer.GetExecuteTime() > 25)
			{
				LOG(WARNING) << "Scene[" << GetMapID() << "] object update cost too long, object id[" << pUpdateObject->GetObject_ID() << "]";
			}
		}
	}
	...
	if(!m_xWaitRemovePlayers.empty())
	{
		...
		for(begiter; begiter != enditer; ++begiter)
		{
			if((*begiter)->GetType() == SOT_HERO)
			{
				...
				//	lua call
				HeroObject* pHero = (HeroObject*)*begiter;
				m_xSceneEventExecutor.Call_ScenePlayerLeave(this, pHero->GetLocateScene(), pHero);
			}
			else if((*begiter)->GetType() == SOT_MONSTER)
			{
...
bool GameScene::LoadScript()
{
	...
	tolua_pushusertype(m_xScript.GetVM(), this, "GameScene");
	lua_setglobal(m_xScript.GetVM(), "SCENEREF");
...
//////////////////////////////////////////////////////////////////////////
int GameScene::CreateMonsterCallback(int _nID, int _nType, int _nPosX, int _nPosY)
{
	switch (_nType)
	{
	case kMonsGenerateDefault:
		{
			return CreateMonster(_nID, _nPosX, _nPosY);
		}break;
	case kMonsGenerateNormal:
		{
			return CreateNormalMonster(_nID, _nPosX, _nPosY);
		}break;
	case kMonsGenerateElite:
		{
			return CreateEliteMonster(_nID, _nPosX, _nPosY);
		}break;
	case kMonsGenerateLeader:
		{
			return CreateLeaderMonster(_nID, _nPosX, _nPosY);
		}break;
	}
...
void GameScene::AutoGenerateMonster()
{
	...
	if (0 == m_xMonsGenEngine.GetGenRecordCount())
	{
		return;
	}

	// new version
	m_xMonsGenEngine.DoGen(m_xMonsCountRecorder);
	return;
...
void GameScene::AllHeroFlyToHome()
{
		...
		++iter)
	{
		pHero = (HeroObject*)iter->second;
		pHero->FlyToHome();
		pHero->SlavesFlyToMaster();
	}
}

void GameScene::CopyAdditionPointCalcFromWorld()
{
	WeightCalc& refWorldCalc = GameWorld::GetInstance().GetAdditionPointCalc();
	const WeightCalcItemVector& refWorldWeightData = refWorldCalc.GetWeightCalcItems();

	// reset first
	m_xAdditionPointCalc.Reset();
	for (size_t i = 0; i < refWorldWeightData.size(); ++i)
	{
		const WeightCalcItem& refItem = refWorldWeightData[i];
		if (refItem.nID == 0)
		{
			// invalid item
			continue;
		...
		m_xAdditionPointCalc.AppendWeight(refItem.nWeight, refItem.nValue);
	}
}

void GameScene::SetAdditionPointWeight(int _nValue, int _nWeight)
{
	m_xAdditionPointCalc.SetWeightByValue(_nValue, _nWeight);
}

void GameScene::OnPlayerEnterScene()
{

}

void GameScene::OnPlayerLeaveScene(GameScene* _pNewScene)
{
