...
void IceGuardMonster::DoAction(unsigned int _dwTick)
{
...
bool IceGuardMonster::CanAttack()
{
	if(m_nAttackMode == -1)
	{
		if(GetObject_HP() > 1800)
		{
...
bool IceGuardMonster::AttackTarget()
{
	bool bRet = false;
...
void IceGuardMonster::AttackHero()
{
	if(m_pTarget == NULL)
	{
		return;
...
bool IceGuardMonster::ExtAttack()
{
	if(NULL == m_pTarget)
	{
		return false;
