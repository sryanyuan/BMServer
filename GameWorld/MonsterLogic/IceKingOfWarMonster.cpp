...
void IceKingOfWarMonster::DoAction(unsigned int _dwTick)
{
	...
	{
		if(m_dwCurrentTime - m_dwLastSpellTime2 >= 800)
		{
...
bool IceKingOfWarMonster::CanAttack()
{
	if(m_nAttackMode == -1)
	{
	...
	//if(m_nAttackMode == 0)
	{
		if(m_dwCurrentTime - m_dwLastSpellTime > 8000)
		{
			//bNeedFly = true;
		}
		else
	...
	{
		if(m_dwCurrentTime - m_dwLastSpellTime2 > 8000)
		{
			//bNeedFly = true;
		}
		else
	...
	//else
	{
		if(m_dwCurrentTime - m_dwLastAttackTime > 8000)
		{
			//bNeedFly = true;
		}
		else
	...
	if(bNeedFly)
	{
		/*if(m_dwCurrentTime - m_dwLastFlyTime > 15000)
		{
			DWORD dwPos = 0;
...
void IceKingOfWarMonster::AttackHero()
{
	if(m_pTarget == NULL)
	{
		return;
	...
	else
	{
			...
			m_nAttackMode == 1)
		{
			if(m_dwCurrentTime - /*m_stData.*/m_dwLastSpellTime > 800)
			{
				...
				{
					if(m_dwCurrentTime - m_dwLastFlyTime > 40000)
					{
						DWORD dwPos = 0;
						if(GetLocateScene()->GetRandomPosition(&dwPos))
						{
...
bool IceKingOfWarMonster::AttackTarget()
{
	bool bRet = false;
...
bool IceKingOfWarMonster::ExtAttack()
{
	//	range attack 3*3
	if(NULL == m_pTarget)
	{
...
bool IceKingOfWarMonster::ExtAttack2()
{
	//	range attack 9*9
	if(NULL == m_pTarget)
	{
